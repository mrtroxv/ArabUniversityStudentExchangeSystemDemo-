...
const StudentDetails = ({ stepper, onStoreData, data }) => {
  ...
  const Schema = Yup.object().shape({
    ...
    stu_sex: Yup.string()
      .oneOf(
        studentGenderTypes.map((value) => value.value),
        "Student Sex type is not valid"
      )
      .required("You should pick one"),
    stu_level: Yup.string()
      .oneOf(
        studentLevelTypes.map((value) => value.value),
        "Student Level is not valid"
      )
      .required("You should pick one"),
