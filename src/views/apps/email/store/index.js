...
export const getMails = createAsyncThunk("appEmail/getMails", async () => {
  console.log("getMails")
  // const response = await axios.get("http://localhost:3500/email/get-emails", {
  //   headers: {
  //     authorization: JSON.parse(localStorage.getItem("accessToken"))
  //   }
  // })

  const response = await axios.get("http://localhost:3500/email/apps/email", {
    headers: {
      authorization: JSON.parse(localStorage.getItem("accessToken"))
    }
  })
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getMails.rejected, (state, action) => {
        console.log("getMails rejected", action.payload)
      })
